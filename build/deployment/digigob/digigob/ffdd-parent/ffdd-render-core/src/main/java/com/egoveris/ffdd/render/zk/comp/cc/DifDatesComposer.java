...
public class DifDatesComposer extends ComplexComponentComposer {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(DifDatesComposer.class);
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		try {
			...
			feriados = ((List<Date>) method.invoke(service))
					.stream().map(input -> {
		...
		this.fechaInicio.addEventListener(Events.ON_CHANGE, (e) 
				-> {countBusinessDaysBetween(feriadosOpt);});
		this.fechaFin.addEventListener(Events.ON_CHANGE, (e) 
				-> {countBusinessDaysBetween(feriadosOpt);});
	}
	...
	private void validateFechas(Optional<List<LocalDate>> holidays) {		
		if (validateFechaFinBefore()) {
			...
			diferencia.setRawValue(null);
			throw new WrongValueException(this.fechaInicio, "La fecha de inicio no debe ser posterior a la fecha de fin");
		} else if (validateFechaInicio(holidays)) {
			diferencia.setRawValue(null);
			throw new WrongValueException(this.fechaInicio, "La fecha de inicio no debe ser anterior a más de 3 días hábiles de la fecha actual");
		}
	}
