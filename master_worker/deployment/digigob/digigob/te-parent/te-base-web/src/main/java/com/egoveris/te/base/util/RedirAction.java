...
public class RedirAction {
  ...
  /**
   * retorna un Map, en una entrada la palabra clave ej: "tareas" y las demas
   * entradas son los ID (ID1 , ID2 ,.., IDn) n se define en la constante
   * CANTIDAD_ID_PERMITIDOS
   * 
   * @param
  ...
  public Map<String, Object> obtenerVariablesRedireccion(ActionContext ac) {
    ...
    try {
      ...
      if (fullPathSplited.length <= 3 + ConstantesWeb.CANTIDAD_ID_PERMITIDOS) {
        hm = new HashMap<>();
        this.keyWord = fullPathSplited[POSICION_PALABRA_CLAVE];
        hm.put(ConstantesWeb.KEYWORD, this.keyWord);
